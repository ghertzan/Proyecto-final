...
function createCartView() {
  ...
  if (!cart || cart.length == 0) {
    ...
    cart.forEach((e) => {
      ...
      cartItemDiv.innerHTML = `
                ...
                <div>
                    ...
                    <span class="cantidad-item-carrito texto-carrito"> ${
                      e.cantidad
                    } </span>
                    <button class="btn btn-sm suma"> > </button>
                </div>
                <h5 class="texto-carrito">Precio: ${toCurrency(
                  e.item.precio
                )}</h5>
                <h5 class="texto-carrito">Total: ${toCurrency(
    ...
    footerDiv.innerHTML = `
            <h5 class="texto-carrito">Total del carrito: ${toCurrency(
              cart.reduce(
